...
class IBCardCheck(rfm.RunOnlyRegressionTest):
    ...
    executable = 'ibstatus'  # changed instead of 'ibstat' then revised the regex below
    ...
    def validate_results(self):
        ...
        regex = r'Infiniband device\s+(?P<device_name>\S+)(.|\n)*?4:\s+(?P<device_state>\S+)(.|\n)*?5:\s+(?P<device_pstate>\S+)(.|\n)*?rate:\s+(?P<device_rate>\S+)(.|\n)*?link_layer:\s+(?P<device_type>\S+)'
